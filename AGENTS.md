...
- Browser-side export libraries are vendored under `assets/vendor/` (`html-to-image.js`, `jspdf.umd.min.js`, `qrcode.min.js`, `me_report.js`) and loaded via `rx.script(src="/vendor/<file>.js")` so the app works offline.
- **NEVER WRITE ANYTHING INTO `.web/`. THE ENTIRE `.web/` DIRECTORY IS DELETED AND REGENERATED FROM SCRATCH ON EVERY `reflex run`. ANY FILE PLACED THERE WILL BE PERMANENTLY LOST. ALL STATIC ASSETS MUST LIVE UNDER `assets/` AND ALL SOURCE CODE UNDER `src/`.**
