...
- The shareable-report URL encodes state as `/materialization?report=1&name=<b64>&cats=<bitmask>` and may include `genes=<b64-json-list>` for exact selected genes; `uv run preselect` uses this path, and `apply_shared_report` re-seeds `ComposeState` deterministically without server persistence. Old `/?tab=sculpture&…` URLs are redirected by `AppState.redirect_legacy_tab`.
