...
- **JavaScript Dependencies**: Use `rx.script(src="CDN_URL")` to load third-party JS libraries from CDN rather than downloading/vendoring them with curl. Only vendor JS when offline operation is a hard requirement (e.g., the existing report export libs in `assets/vendor/`). For visualization/UI libraries that tolerate a network dependency, prefer CDN loading — it's the native Reflex pattern and avoids committing large minified blobs.
...
- Protein structure PDB files live in `assets/structures/` (committed) and are served at `/structures/` by Reflex's built-in asset serving. Files are named `{PDB_ID}.pdb` for experimental structures or `{UNIPROT_ID}_predicted.pdb` for AlphaFold predictions. `gene_data.py:resolve_structure_pdb()` resolves `gene_id` → local filename; the field is `structure_pdb` on `GeneEntry` / `SculptureSelectedGene`. 3Dmol.js is loaded from CDN via `rx.script(src=...)` and a MutationObserver auto-initializes `.me-pdb-viewer` divs when they appear.
